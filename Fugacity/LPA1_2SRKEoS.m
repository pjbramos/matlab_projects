...
% Parameter constants for SRK EoS
  psi = 0.42748;
  omega = 0.08664;
  sigma = 1;
  epsilon = 0;
...
% Determine the parameter alpha in the SRK EoS for each species in the system
  alpha_srk = zeros(1,n);
  for b = 1:n
      alpha_srk(1,b) = (1 + (0.480 + 1.574*w(1,b) - 0.176*w(1,b)^2)*(1 - tempr(1,b)^(1/2)))^2;
  end
...
% Determine the values of the dimensionless parameters for pure species (a and b)
  % Solve for ai
    ai = zeros(1,n);
    for b = 1:n
        ai(1,b) = (psi*alpha_srk(1,b)*(83.14472^2)*(tempc(1,b)^2))/(pressc(1,b)); 
    end
  % Solve for bi
    bi = zeros(1,n);
    for b = 1:b
