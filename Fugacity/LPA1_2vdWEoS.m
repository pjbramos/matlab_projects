...
% Parameter constants for vdW EoS
  psi = (27/64);
  omega = (1/8);
  sigma = 0;
  epsilon = 0;
...
% Determine the parameter alpha in the vdW EoS for each species in the system
  alpha_vdW = ones(1,n);
...
% Determine the values of the dimensionless parameters for pure species (a and b)
  % Solve for ai
    ai = zeros(1,n);
    for b = 1:n
        ai(1,b) = (psi*alpha_vdW(1,b)*(83.14472^2)*(tempc(1,b)^2))/(pressc(1,b)); 
    end
  % Solve for bi
    bi = zeros(1,n);
    for b = 1:b
