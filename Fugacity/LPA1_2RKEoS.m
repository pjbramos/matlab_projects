...
% Parameter constants for RK EoS
  psi = 0.42748;
  omega = 0.08664;
  sigma = 1;
  epsilon = 0;
...
% Determine the parameter alpha in the RK EoS for each species in the system
  alpha_rk = zeros(1,n);
  for b = 1:n
      alpha_rk(1,b) = (tempr(1,b)^(-1/2));
  end
...
% Determine the values of the dimensionless parameters for pure species (a and b)
  % Solve for ai
    ai = zeros(1,n);
    for b = 1:n
        ai(1,b) = (psi*alpha_rk(1,b)*(83.14472^2)*(tempc(1,b)^2))/(pressc(1,b)); 
    end
  % Solve for bi
    bi = zeros(1,n);
    for b = 1:b
